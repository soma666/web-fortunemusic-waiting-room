 ...
 *
 * 历史数据 API 端点，使用 Upstash Redis Sorted Sets 存储数据。
 *
 * 数据结构：
 * - history:ts:{day}:{eventId}:{sessionId} → Sorted Set (score=timestamp, member=JSON record)
 * - history:day:{day} → JSON day index (活动/场次摘要)
 *
 * 支持的操作：
 * - GET mode=days: 返回有数据的日期列表
 * - GET mode=events: 返回某日内的活动/场次摘要
 * - GET mode=details: 返回某日某活动的详细时间序列
 * - GET (默认): 兼容旧版查询
 * - POST: 批量保存历史记录
 * - DELETE: 删除历史记录
 */
...
/** 将 ZRANGE 返回的成员解析为 HistoryRecord（兼容自动反序列化和原始字符串） */
...
async function handleGetLegacy(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    for (const key of keys as string[]) {
      // 从 key 名称快速过滤 eventId/sessionId
...
async function handlePost(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    for (const [groupKey, groupRecords] of groups) {
      const [eid, sid] = groupKey.split(':').map(Number);
